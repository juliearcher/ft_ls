...
static void	ft_putchar_unicode2(wchar_t c, t_printf *str)
{
	...
	{
		ft_putchar(((c & 0xf000) >> 12) + 0xe0, str);
		ft_putchar(((c & 0x0fc0) >> 6) + 0x80, str);
		ft_putchar((c & 0x003F) + 0x80, str);
		str->precision -= 3;
	}
	else if (str->precision <= -1 || str->precision >= 4)
	{
		ft_putchar(((c & 0x1c0000) >> 18) + 0xf0, str);
		if (--str->precision == 0)
			return ;
		ft_putchar(((c & 0x3f000) >> 12) + 0x80, str);
		if (--str->precision == 0)
			return ;
		ft_putchar(((c & 0x0fc0) >> 6) + 0x80, str);
		if (--str->precision == 0)
...
void		ft_putchar_unicode(wchar_t c, t_printf *str)
{
	...
	{
		ft_putchar(((c & 0x07c0) >> 6) + 0xc0, str);
		if (--str->precision == 0)
			return ;
		ft_putchar((c & 0x003f) + 0x80, str);
		--str->precision;
